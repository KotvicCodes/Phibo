...
1. Add more time labels to the Tags day strip. It only prints a month
...
2. Review the view-switch state resets introduced by the component split:
...
3. Discoveries anchor date falls back to the sync form's end date
   (InsightsView gets endDate as a prop just for this). Falling back to
   today's date instead would cut the odd coupling.
4. "Delete local data" wipes IndexedDB but not localStorage: Optimal
