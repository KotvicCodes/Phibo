...
function invertSymmetric(matrix: number[][]): number[][] | null {
  const size = matrix.length
  const inverse: number[][] = []
  for (let col = 0; col < size; col += 1) {
    const unit = new Array<number>(size).fill(0)
    unit[col] = 1
    const column = solveSymmetric(matrix, unit)
...
export function fitRidge(
  ...
  const standardizedCoefficients = solveSymmetric(penalized, zty)
  if (standardizedCoefficients === null) return null
  ...
  // Sandwich covariance on the standardized scale.
  const penalizedInverse = invertSymmetric(penalized)
  if (penalizedInverse === null) return null
...
function predictionError(
  ...
  const fit = fitRidge(trainRows, trainOutcomes, { lambda })
